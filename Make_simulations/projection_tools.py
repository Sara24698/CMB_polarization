...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun  9 17:24:26 2023

@author: josemanuel
"""

# from Longitude_Latitude_Calculator import reading_longitude_latitude_file
...
def make_mosaic_from_healpix( hpxmap , Npix, pixel_size , overlap=5 *u.deg , Number_Of_Patches = 1, adaptive=False  ):
    """
    ...
    **Returns**
    ...
    # from numpy import sqrt, log, pi, arccos, random, cos
    
    # Number_Of_Simulations = Number_Of_Patches
    ...
    #     r1, r2, r3 = random.rand(3,1)
    #     Longitude = r1 * 360.
    #     Latitude = arccos(cos((30. - 90.) / 180. * pi) * (1 - r2) + r2)
        
    #     Latitude *= 180. / pi  # lat in degrees
    #     Latitude -= 90.  # colat
    #     Latitude *= r3.round() * 2 - 1
        
    #     lon = np.append(lon, Longitude)
    #     lat = np.append(lat, Latitude)
    
    lon, lat = reading_longitude_latitude_file()
...
def make_mosaic_from_healpix_with_random_positions( hpxmap , pixel_size , Npix=256, overlap=5 *u.deg , Number_Of_Patches = 1, adaptive=False, lon=45., lat=45.  ):
    """
    Make a mosaic of patches from a healpix map.
    **Parameters**
    - `hpxmap`:{array}
    ...
    **Returns**
    ...
    # for phi,theta in zip (lon , lat   ):
    header = set_header(lon, lat, pixel_size.value, Npix)
    patches.append(h2f(hpxmap , header))
